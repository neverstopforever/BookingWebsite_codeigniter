...
function setCurrentFont(){
  if (strlen($this->currentBaseFont)==0){
    ...
    $this->selectFont('./r/fonts/pdf-fonts/fonts/Helvetica.afm');
  }
...
function stream($options=''){
  ...
  $fileName = (isset($options['Content-Disposition'])?$options['Content-Disposition']:$options['filename'].'.pdf');
  $type=$options['type'];
...
function getFontHeight($size){
  if (!$this->numFonts){
    $this->selectFont('./r/fonts/pdf-fonts/fonts/Helvetica');
  }
...
function addText($x,$y,$size,$text,$angle=0,$wordSpaceAdjust=0){
  if (!$this->numFonts){$this->selectFont('./r/fonts/pdf-fonts/fonts/Helvetica');}
